...
- **서버 제어** - 시작 / 정지(공지 후 종료) / 강제 종료 / 재시작. 접속자 있을 때 종료 시 **종료 대기 시간(1~300초)** 을 웹에서 설정 가능.
- **설정 편집** - PalWorldSettings.ini를 슬라이더·토글·셀렉트로 편집 (카테고리별 분류). **설정 검색**(한글 이름/영문 키) 지원. 공식 문서에 없는 항목은 `[공식문서에없음]` 표시.
...
- **(선택) 서버 시작 시 Chrome 자동 오픈** - Windows에서 패널 기동 시 브라우저 자동 실행. `.env`에서 `OPEN_BROWSER=false`로 끌 수 있음.
...
├── logs/
│   └── panel.log       # 패널 동작 로그 (서버 제어·설정 저장 등 전부 기록)
├── player_list.txt      # 접속했던 플레이어 목록 (레거시, DB와 병행)
├── playtime.txt         # 누적 플레이타임 (레거시, DB와 병행)
├── panel_config.json   # 패널 전용 설정 (예: 공지 후 종료 대기 시간)
...
| `OPEN_BROWSER` | 서버 시작 시 Chrome으로 패널 URL 자동 오픈 (Windows). 끄려면 `false` 또는 `0` | 켜짐 |

- **아이콘:** 헤더/로그인 큰 아이콘은 `.env`의 `PANEL_ICON`(이모지 1개)으로 변경. 브라우저 탭 아이콘은 `public/favicon.svg` 또는 `public/favicon.ico`로 적용됩니다.
...
| 상황 | 동작 |
|------|------|
| 접속자 없음 | 확인 후 바로 종료 (REST API 사용 시 30초 후 종료 요청, 35초 후에도 살아 있으면 강제 종료) |
| 접속자 있음 | 종료 확인 모달 표시 → **종료 대기 시간(1~300초, 기본 60초)** 을 웹에서 설정 가능 → 공지 후 해당 초 뒤 종료. 10초 미만이면 배너에 즉시 "N초 뒤 종료" 카운트다운 표시. 70초(또는 설정초+10초) 후에도 프로세스가 살아 있으면 강제 종료 |
...
웹 패널에서 편집 가능한 PalWorldSettings.ini 설정 항목. **설정 검색**으로 한글 이름 또는 영문 키로 필터링할 수 있습니다. 공식 문서에 없는 항목은 라벨 뒤에 `[공식문서에없음]`으로 표시됩니다.

| 카테고리 | 설정 예시 |
|----------|-----------|
| 기본설정 | 서버 이름, 비밀번호, 포트, 최대 인원, RCON, REST API, 채팅 제한, 크로스플레이 |
| 접속·채팅 | 월드 백업 사용, 채팅 분당 제한, 크로스플레이 플랫폼, 클라이언트 모드 허용, ESC 플레이어 목록 |
| 전투 | 팰/플레이어 공격력·방어력, PvP, 사망 페널티, 리스폰 대기, 장비 내구도 |
| 사망·리스폰 | 리스폰 대기 시간, 사망 시 팰 영구 손실, 리스폰 페널티 |
| 캡처 | 포획률, 스폰 배율, 미접속 페널티 |
| 경험치/드랍 | 경험치 배율, 알 부화 시간, 채집·적 드랍률 |
| 생존 | 배고픔·스태미나 감소율, HP 자동회복 |
| 길드 | 길드 최대 인원, **길드당 거점 최대 수**(공식), 거점 배치 팰 수 |
| 성능 | 길드 재가입 쿨타임, 플레이어당 건물 수 제한, 팰 동기화 거리 |
| 기타 | 낮/밤 속도, 건축물 피해, 멀티플레이, PvP 모드, 비활성 길드 리셋 |
| 편의·이벤트 | 순간이동, 거점 간 순간이동만, 로그아웃 시 위치 유지, 하드코어, 침공자 이벤트, 메테오 간격, 아이템 무게·부패 |

서버 실행 중에는 REST API에서 현재 적용된 값을 가져오고, 정지 상태에서는 ini 파일에서 읽습니다.

## 로그

패널 화면의 로그 영역에는 서버 시작/정지 등 동작이 최근 50줄까지 표시됩니다. **동일 내용이 `data/logs/panel.log` 파일에도 계속 추가**되므로, 디버깅·이력 확인 시 해당 파일을 열어보면 됩니다.
...
| `[복원]` | 패널 시작 시 세션 복원 |
| `[정지디버그]` | 서버 정지 시 tasklist PID 등 디버그 정보 |

- 웹 UI에는 최근 50줄만 표시되며, **모든 로그는 `data/logs/panel.log`에 전부 기록**됩니다.
